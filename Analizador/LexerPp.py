...
class Lexer:
    def __init__ (self):
        self.reservada_keywords = ['do','while','if', 'else', 'while', 'for', 'int', 'float', 'Cadena', 'print', 'programa', 'read',
                                   'terminar', 'imprimir', 'public', 'static', 'void', 'main']
        self.Simboloss = ['+', '-', '*', '/', '=', '==', '!=', '<', '>', '<=', '>=', '(', ')', '{', '}', ';', ',', '"',
                          "'"]
        self.token_patterns = [
            ('Cadena', r'"(?:[^"\\]|\\.)*"'),
...
class LexerApp:
    def __init__ (self):
        ...
        self.analyze_button = tk.Button (self.button_frame, text="Analizar", command=self.analyze_text, bg="#0000CD",
                                         font=("Times", 14, 'bold'))
        self.analyze_button.grid (row=0, column=0, padx=30, pady=10)

        # crea un boton para limpiar el cuadro de texto
        self.clean_button = tk.Button (self.button_frame, text="Limpiar", command=self.clean_text, bg="#4000FF",
                                       font=("Times", 14, 'bold'))
        self.clean_button.grid (row=0, column=1, padx=30, pady=10)

        # Crea un boton para salir del programa
        self.exit_button = tk.Button (self.button_frame, text="Salir", command=self.exit_app, bg="#00008B",
                                      font=("Times", 14, 'bold'))
        self.exit_button.grid (row=0, column=2, padx=30, pady=10)

        self.tree = ttk.Treeview (self.windows, columns=(
        "Linea", "Token", "Funcion", "Reservada", "Cadena", "Identificador", "Símbolo", "Numero"), show="headings")
